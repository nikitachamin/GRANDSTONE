 export default async (req, res) => {
    ...
    try {
      ...
      if (!agreement) return res.status(400).json({ error: 'Не принято соглашение' });
      ...
      if (!data.ok) {
        return res.status(500).json({ error: 'Ошибка при отправке сообщения в Telegram', detail: data });
      }
